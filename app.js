...
function sumAndMultiply(a, b, c) {
  var sumProblem3 = sum(sum(a, b)[0],c)[0];
  var productProblem3 = multiply(multiply(a, b)[0],c)[0];
  var thirdString = a + ' and ' + b + ' and ' + c + ' sum to ' + sumProblem3 + '.';
  var fourthString = 'The product of ' + a + ' and ' + b + ' and ' + c + ' is ' + productProblem3 + '.';
  return [sumProblem3,productProblem3,thirdString,fourthString];
}
...
function sumArray(sumArr) { //eslint-disable-line
  ...
  for (var index = 0; index < sumArr.length; index++) {
    result = sum(result, sumArr[index])[0];
    if (index === 0) {
      myString = myString + sumArr[index];
    } else {
      myString = myString + ',' + sumArr[index];
    }
  }
  myString = myString + ' was passed in as an array of numbers, and ' + result + ' is their sum.';
...
function multiplyArray(multArr) { //eslint-disable-line
  var multiString = 'The numbers ';
  var results = 1;
  for(var index = 0; index < multArr.length; index++) {
    results = multiply(results, multArr[index])[0];
    if (index === 0) {
      multiString = multiString + multArr[index];
    } else {
      multiString = multiString + ',' + multArr[index];
    }
  }

  multiString = multiString + ' have a product of ' + results + '.';

  return [results,multiString];
}
